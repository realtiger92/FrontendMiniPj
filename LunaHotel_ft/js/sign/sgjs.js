...
window.onload = function () {
    ...
    join.onsubmit = function () { //join에서 submit이 실행된다면 수행할 함수           
        ...
        if (!idLimit.test(input[0].value)) {
            document.getElementById(errorId[0]).innerHTML = " 5~14자의 영문 소대문자, 숫자와 특수기호(_),(-)만 사용 가능합니다.";
        ...
        if (!pwLimit.test(input[1].value)) {
            document.getElementById(errorId[1]).innerHTML = " 9~14자의 영문 소대문자, 숫자와 특수기호 '~!@#$%^&*()_-'만 사용 가능합니다.";
        ...
        alert("회원가입이 완료되었습니다. 루나 멤버쉽 회원이 되신 것을 환영합니다!!");
